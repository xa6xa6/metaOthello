...
int main(int argc, char** argv)
{
	if((argc != 5)&&(argc != 6))
	{
		...
		cout << "#1 outputReadBaseNumFile" << endl;
		cout << "#2 SE_or_PE" << endl;
		cout << "#3 fa_or_fq" << endl;
		cout << "#4 seReadFile or peReadFile_1" << endl;
		cout << "#5 (peReadFile_2)" << endl;
		exit(1);
	}	
	string SE_or_PE_str = argv[2];
	string fa_or_fq_str = argv[3];
	bool SE_or_PE_bool;
	if(SE_or_PE_str == "SE")
		SE_or_PE_bool = true;
	...
	if(SE_or_PE_bool && (argc != 5)){
		cout << "error! (SE_or_PE_bool && (argc != 5))" << endl;
		exit(1);
	}
	else if((!SE_or_PE_bool) && (argc != 6)){
	...
	string outputReadBaseNumFile = argv[1];
