...
int main(int argc, char** argv)
{
	...
	while((!readAssignment_ifs.eof())&&(!readFa_ifs.eof()))
	{
		string tmpStr;
        ...
        if(tmpReadSeqStr == "")
        	break;
        int tmpReadSeqLength = tmpReadSeqStr.length();
	...
	double sensi_species = ((double)correct_num_species/(double)total_num) * 100;
	double preci_species = ((double)correct_num_species/(double)mapped_num_species) * 100;
	double sensi_genus = ((double)correct_num_genus/(double)total_num) * 100;
	double preci_genus = ((double)correct_num_genus/(double)mapped_num_genus) * 100;
	double sensi_family = ((double)correct_num_family/(double)total_num) * 100;
	double preci_family = ((double)correct_num_family/(double)mapped_num_family) * 100;
	double sensi_order = ((double)correct_num_order/(double)total_num) * 100;
	double preci_order = ((double)correct_num_order/(double)mapped_num_order) * 100;
	double sensi_class = ((double)correct_num_class/(double)total_num) * 100;
	double preci_class = ((double)correct_num_class/(double)mapped_num_class) * 100;
	double sensi_phylum = ((double)correct_num_phylum/(double)total_num) * 100;
